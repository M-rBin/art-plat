...
> 全部从 `.claude/ByAgent-knowledge/phase3-development/project.md` 读取。
...
**5.1 路由注册**
- 按项目规范（从 `.claude/ByAgent-knowledge/phase3-development/project.md` 或步骤 2 推断）注册路由
...
> **规范库位置**：本地知识库，通过 Read(".claude/ByAgent-knowledge/{category}.md") 加载。
> **加载方式**：Step 2 通过 `spec-loader` agent 按需加载，不会一次性读取所有文件。
> **查看目录**：调用 Read(".claude/ByAgent-knowledge/catalog.json") 获取所有可用 category。
