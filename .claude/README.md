...
> 本目录是「开发态」配置，改动立即在当前仓库生效；确认无误后同步到 `packages/ByAgent/content/` 打包发布。
...
│
├── rules/                           # 行为铁律：会话启动自动加载
│   ├── ByAgent-agent-orchestration.md      # Agent 编排规则
│   ├── ByAgent-coding-conventions.md       # 编码规范
│   ├── ByAgent-frontend-conventions.md     # 前端规范
│   ├── ByAgent-development-workflow.md     # 开发工作流
│   ├── ByAgent-req-doc-workflow.md         # 需求说明书工作流
│   ├── ByAgent-git-workflow.md             # Git 工作流
│   ├── ByAgent-session-context.md          # 会话上下文与强制清单
│   ├── ByAgent-compaction-recovery.md      # 上下文压缩恢复
│   ├── ByAgent-spawn-subagent.md           # 子 Agent 调用规则
│   ├── ByAgent-subagent-stop.md            # 子 Agent 技能触发禁止
│   └── ByAgent-tool-call-integrity.md      # 工具调用完整性
│
...
│   └── lib/                         #   公共库
│
├── ByAgent-knowledge/               # 分阶段知识库（按需 Read，线上 MCP 副本）
...
| `ByAgent-knowledge/` | 分阶段详细规范——按需 Read 的知识库本地副本 | ✅ |
