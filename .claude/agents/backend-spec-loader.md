...
- `.claude/ByAgent-knowledge/conventions/coding.md`（编码规范，生成任何代码前必须加载）
- `.claude/ByAgent-knowledge/conventions/security.md`（安全：脱敏、注入防护、鉴权）
- `.claude/ByAgent-knowledge/phase3-development/backend.md`（接口设计原则，框架无关）
- `.claude/ByAgent-knowledge/phase3-development/database.md`（数据库设计原则，框架无关）

### 第 3 步：加载框架特有规范（命中预置则读，否则从代码推断）

根据第 1 步识别的框架，查 `.claude/ByAgent-knowledge/phase3-development/backend-framework/` 是否有对应子目录：
