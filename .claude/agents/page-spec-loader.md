...
读取 `.claude/ByAgent-knowledge/` 目录下对应规范文件，**并行**获取以下规范：
...
若需要项目特有规范，读取 `{PROJECT_PATH}/.claude/ByAgent-knowledge/` 下的文件（如有）。
...
根据第 1 步识别的 UI 库，调用 Read(".claude/ByAgent-knowledge/catalog.json") 查看是否有对应规范，有则加载，无则从项目现有代码推断：

```
有对应 category → 用 Read 工具加载对应 .claude/ByAgent-knowledge/ 文件
...
调用 Read(".claude/ByAgent-knowledge/catalog.json") 查看是否有对应 UI 库的页面规范，有则加载，无则从项目现有代码推断：
