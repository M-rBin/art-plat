...
3. **SRS 规范扫描** — 修改需求说明书前必须读取规范，修改后必须执行禁用词扫描+跨章节一致性检查。不通过不能报告完成。详见 `ByAgent-req-doc-workflow.md`。
...
| 层级 | 机制 | 内容 |
|------|------|------|
| 自动加载 | `.claude/rules/ByAgent-*.md` | 核心编码规范、前端规范、工作流规则 |
| 按需读取 | `.claude/ByAgent-knowledge/` | 详细规范（测试、安全、TypeScript 等） |

何时 Read ByAgent-knowledge/：
