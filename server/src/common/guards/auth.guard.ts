...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // C 端（/app/* 或 /api/auth、/api/profile、/api/documents）走独立鉴权体系，
    // 由 ArtistAuthGuard 负责校验，全局 AuthGuard 放行。
    if (this.isAppRoute(request) || this.isArtistRoute(request)) {
      this.logger.debug(`[C端路由放行] ${request.url}`);
      return true;
    }
